...
const BinarySearchVisualization = () => {
  ...
  const resetSearch = () => {
    const newArray = Array.from({ length: 7 }, () => Math.floor(Math.random() * 100)).sort((a, b) => a - b);
  ...
  return (
    <div className="p-4 sm:p-6 bg-gradient-to-r from-blue-100 to-purple-100 rounded-xl shadow-lg max-w-4xl mx-auto">
      <h2 className="text-2xl sm:text-3xl font-bold mb-4 text-center text-indigo-700">二分法探索</h2>
      
      <div className="bg-white p-4 sm:p-6 rounded-lg shadow mb-6">
        <h3 className="text-xl sm:text-2xl font-bold mb-2 text-indigo-600">二分法探索とは？</h3>
      ...
      <h3 className="text-xl sm:text-2xl font-bold mb-4 text-center text-indigo-600">二分法探索を見てみる</h3>
      <p className="text-lg sm:text-xl mb-4 text-center font-semibold">探したい数: <span className="text-xl text-red-500">{target}</span></p>
      
      <div className="flex flex-wrap justify-center gap-2 sm:gap-4 mb-6 transition-all duration-300">
        {array.map((num, index) => (
          <div
            key={index}
            className={`w-8 h-8 sm:w-12 sm:h-12 flex items-center justify-center rounded-lg text-sm sm:text-base font-bold transition-all duration-300 ${
      ...
      <div className="flex justify-center gap-2 sm:gap-4">
        <button
          ...
          className="px-4 sm:px-6 py-2 sm:py-3 bg-blue-500 text-white rounded-full text-base sm:text-lg font-semibold hover:bg-blue-600 disabled:bg-gray-400 transition-colors duration-300"
        >
          次のステップ
        </button>
        <button
